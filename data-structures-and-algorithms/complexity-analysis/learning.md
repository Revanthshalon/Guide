...
- **Trade-offs of the fix:** A `HashSet` costs a hash per probe and loses cache locality, so at very small n a linear scan wins — but the window is narrower than folklore claims. Measured with `u32` keys, `HashSet::contains` overtakes `Vec::contains` at about **n = 12** and is *flat* thereafter (≈10 ns) while the scan grows linearly (90 ns at n = 1024). Expensive keys or out-of-cache tables push that crossover up. The point isn't "always use a set" — it's that the choice must be *made*, and that the intuition favouring flat arrays is calibrated too high.

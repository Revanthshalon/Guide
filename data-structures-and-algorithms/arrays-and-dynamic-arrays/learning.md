...
- **Why it happens (the mechanism):** Θ(n) vs Θ(1) is a compelling argument that omits the constants. A contiguous scan of 200 `u32`s is 800 bytes — 13 cache lines, fully prefetched, a handful of nanoseconds. A `HashMap` lookup is a hash (measured: ~12 ns for a 4-byte key with SipHash — dominated by fixed setup cost, not by per-byte throughput), a random probe into a table that isn't in cache, and a comparison.

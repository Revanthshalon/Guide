...
### Pitfall: Hot-aggregate contention (the retry storm)

- **What goes wrong:** One aggregate receives a burst of concurrent commands — a flash-sale product, a viral post's like counter, a company-wide ledger account. Optimistic concurrency admits exactly one append per version: of N concurrent commands, one wins and N−1 conflict, rehydrate, and retry — colliding again with the next wave. Effective throughput collapses toward *worse* than serial (each retry re-reads the whole stream), latency explodes, and the retries themselves amplify load on the store: a self-sustaining storm precisely when traffic peaks.
- **Why it happens (the mechanism):** `expected_version` makes the aggregate a single serialization point — that's its job. Optimistic concurrency is optimistic: it bets conflicts are rare, and settles conflicts by *redoing work*. When contention stops being rare, the redo cost dominates. The failure is quadratic-ish (N commands → O(N²) attempted appends in the worst wave), which is why it appears suddenly under load rather than degrading gracefully.
- **How to handle it in production, and why that works:** In escalating order: (1) **Bound and jitter the retry loop** (3–5 attempts, exponential backoff + jitter, then surface failure) — stops the amplification, doesn't fix throughput. (2) **Single-writer routing**: route all commands for one aggregate to one in-process handler (actor per aggregate, or partition command topic by `aggregate_id`) that processes them *serially against an in-memory aggregate* — conflicts vanish because concurrency at the stream is eliminated; the handler appends, applies, and moves on without re-reading. This is the standard high-throughput event-sourcing shape (the actor model earns its keep here). (3) **Batch within the writer**: one append of several events per wave of commands. (4) **Redesign the aggregate**: if commands are commutative, the contention is artificial — split the counter into shards summed by a projection, or use the reservation pattern to pre-partition capacity (100 units = 10 buckets of 10). The deepest fix is noticing that "everyone writes one aggregate" often means the invariant was drawn too wide.
- **Trade-offs of the fix:** Single-writer routing adds infrastructure (sharded routing, rebalancing on deploy, the "where does the actor live" problem) and makes the aggregate's throughput ceiling explicit — one writer's speed. Sharded counters trade the absolute invariant for an eventual one (each shard enforces its slice). Backoff trades latency for stability. All are better than discovering the storm during the flash sale.

### Pitfall: Cross-aggregate ordering and the multi-cursor projection

- **What goes wrong:** A read model joins data from several streams — an order summary needing `orders`, `customers`, and `payments` events. Built naively with **independent subscriptions per stream, each with its own checkpoint**, the projection's inputs advance at different rates: after a crash, the orders cursor has replayed to Tuesday while the payments cursor sits at Monday — the read model shows states that never coexisted (paid orders marked unpaid), and the skew *changes* as cursors catch up independently. A second form hits even single-cursor projections: `OrderPlaced{customer_id: c-77}` arrives before `CustomerRegistered{c-77}` was projected (different streams, no cross-stream ordering promise), and the handler's `UPDATE ... WHERE customer_id = 'c-77'` silently matches zero rows — data quietly missing, no error anywhere.
- **Why it happens (the mechanism):** The event store promises order *within* a stream and (with a global position) a total order of *arrival* — but not causal order across aggregates, and nothing at all across independent consumer cursors. Two cursors are two clocks; any state assembled from both is a torn read. And arrival order across streams reflects commit interleaving, not domain causality — the customer's registration may genuinely commit after the order references it (or merely be projected later). This is the [replication](../replication-and-consistency/learning.md) causal-anomaly problem reborn inside your own read side.
- **How to handle it in production, and why that works:** Rule one: **one read model, one cursor** — a projection consumes a single ordered feed (the store's `$all` / global subscription, filtered to relevant event types) with a single checkpoint, so its state always corresponds to one position in one log; crash-recovery resumes from that position and the torn-cursor problem is structurally impossible. For the missing-reference form: make handlers **tolerant of out-of-causal-order arrival** — upsert placeholder rows (`INSERT ... ON CONFLICT` a stub customer filled in when `CustomerRegistered` lands) or park-and-retry the event (a small parked-events table drained on later arrivals); or remove the dependency by making events **self-contained** (the `OrderPlaced` integration event carries the customer name it was placed with — denormalize at write time, which is often the honest domain model anyway: the order *was* placed with that name). For sagas making cross-aggregate decisions, don't read lagging projections — react to events themselves and let [compensation](../saga-pattern/learning.md) handle the races that remain.
- **Trade-offs of the fix:** One global cursor serializes the projection's ingest (parallelize by *partitioning read models*, not by splitting one model's inputs — same rule as the outbox relay). Placeholder rows admit transiently-incomplete reads (label them). Self-contained events grow payloads and freeze point-in-time facts — usually a feature (audit) but occasionally a modeling argument. Parked-event tables need monitoring, or "parked forever" becomes the new silent loss.

### Pitfall: The retried command that legitimately duplicates (command-level idempotency)

- **What goes wrong:** A client sends `Withdraw{50}`, the response times out, the client retries. The first attempt *succeeded* — the stream is now at v4 — so the retry rehydrates at v4, validates fine (balance allows another 50), and appends a second `MoneyWithdrawn{50}` at v5. `expected_version` never fires: there's no version conflict, just a second, perfectly-valid-looking command. The customer is debited twice and every mechanism in the store agrees this was correct.
- **Why it happens (the mechanism):** Optimistic concurrency protects against *interleaved* writers, not *sequential duplicates* — the retry isn't racing anyone; it arrives after the original completed. This is the [timeout ambiguity](../idempotency-and-delivery-semantics/learning.md) landing on the write side: the version check answers "has the stream moved since I read it?" but the question a retry needs answered is "has *this command* already been processed?" — a different key entirely.
- **How to handle it in production, and why that works:** Commands carry an **idempotency key** minted at the source of intent (one user action = one key), and the append path dedups on it atomically: store recent command ids in the stream itself (event metadata checked during rehydration, or a per-stream `recent_commands` set in the appended state) or in a command-dedup table updated *in the append transaction*. A repeat key returns the original result (the events already written) instead of re-deciding. Same-transaction is the load-bearing phrase — a dedup check against a separate cache reopens the race the [idempotency notes](../idempotency-and-delivery-semantics/learning.md) dissect.
- **Trade-offs of the fix:** Command ids in event metadata grow rehydration state (bound the window — keep the last N or last-T-minutes of ids, matching realistic client retry horizons). A dedup table needs pruning and couples to the store's transaction. Both are cheap next to the alternative; this is the write-side twin of projection idempotency, and it's the most commonly *forgotten* member of the family because the version check feels like it already covers it.

### Pitfall: The zombie projector (two instances of one projection)

- **What goes wrong:** A projection runs highly-available or gets redeployed: instance B takes over while instance A is stalled mid-GC-pause, mid-network-partition, or just slow to shut down. A wakes up and keeps applying events from *its* stale position — interleaving old updates over B's newer ones, corrupting the read model in ways that look like random staleness. The split-brain problem, miniaturized into your read side.
- **Why it happens (the mechanism):** "Exactly one consumer per projection" is assumed but nothing *enforces* it — deploys overlap, health checks lie, pauses outlast leases. A stalled process cannot know it was replaced ([the fencing problem](../consensus-and-leader-election/learning.md)); when it resumes, its writes are as confident as ever.
- **How to handle it in production, and why that works:** Make the **checkpoint update the fence**: `UPDATE checkpoint SET position = $new WHERE projection = $p AND position = $expected` in the same transaction as the projection writes — the zombie's update matches zero rows (its `$expected` is stale), the transaction aborts, and its writes never land. This piggybacks split-brain protection on machinery you already have, with no leases or leader election. Where the read store lacks transactions, use an epoch/generation number: each takeover increments it, every write carries it, stale-epoch writes are rejected.
- **Trade-offs of the fix:** The CAS checkpoint serializes each projection to one effective writer (that was the intent). The zombie dies loudly (aborted transactions in its logs) — wire that signal to alerting rather than letting it retry forever. Epoch schemes need a place to atomically increment the epoch, which is a small [consensus](../consensus-and-leader-election/learning.md) dependency.

### Pitfall: Rebuilding a projection while the world keeps writing

- **What goes wrong:** "Projections are disposable — reset the checkpoint and replay" meets production: the rebuild takes four hours, during which the read model serves *partially rebuilt* state (Tuesday's orders present, Wednesday's missing — worse than uniformly stale, because it's inconsistently wrong), or the screen is down entirely. And at the end, the cutover races the live stream: the rebuild reaches "now," but *now keeps moving* — switch too early and recent events are missing; the naive loop never quite catches a busy stream.
- **Why it happens (the mechanism):** Replay is described as an offline operation, but the log is live — rebuild is really *bootstrapping a new replica while the leader keeps writing*, the same catch-up-and-handoff problem as [CDC's snapshot-plus-stream](../change-data-capture/learning.md) and database replica provisioning. It needs an explicit convergence-and-cutover design, and "reset the checkpoint" doesn't contain one.
- **How to handle it in production, and why that works:** **Blue/green the read model**: build `orders_summary_v2` alongside `v1`, which keeps serving untouched. The v2 projector replays from zero and *keeps consuming* — it doesn't need to "finish," only to converge (lag below a threshold, since a live projector at position P chasing a head at P+ε is just a normal projection). Then cut over atomically at the serving layer (SQL view repointed, alias swap in Elasticsearch, config flip), and drop v1 after soak. Reads never see partial state; the "catch the moving head" problem dissolves because v2 is simply a second subscriber that goes live when close enough.
- **Trade-offs of the fix:** Double storage and double write-load during the rebuild window; the serving layer needs an indirection (view/alias) worth installing on day one. Read-your-writes guarantees need care across the swap (v2's checkpoint must be ≥ the position a session already observed on v1 — gate the cutover on that same threshold). These costs are why "test rebuild-from-zero in anger" appears in the checklist — the machinery only exists if it's been exercised.
...
- **How to handle it in production, and why that works:** **Crypto-shredding**: encrypt PII fields with a per-person key stored outside the log (the encrypted-payload design in Core Concepts, built on [envelope encryption](../encryption-and-key-management/learning.md)); on erasure, destroy the key — ciphertext remains in the log (integrity preserved) but is unrecoverable, which regulators have accepted as erasure. Alternatively **keep PII out of events entirely**: events carry `user_id`, PII lives in a small mutable store, deletion is a plain delete. Projections re-project or redact on the same trigger. Decide *before* the first PII-bearing event is written.
